     ...
     jQuery('#cmt-loading').delay(3000).fadeOut(3000);
					...
					if(isChild) {
						var s = '<div class="comment-children">';
							s += '<div class="avatar"><img alt="'+ jQuery.trim(jQuery('input[name="comname"]').val()) +'" src="http://gravatar.duoshuo.com/avatar/' + jQuery.md5(jQuery.trim(jQuery('input[name="commail"]').val())) + '?s=36&d=identicon&r=g"></div>';
						s += '<div class="comment-info"><span class="poster"><i class="fa fa-user mar-r-4 green"></i>' + jQuery.trim(jQuery('input[name="comname"]').val()) + '</span><span id="ajax_ok"><i class="fa fa-exclamation-circle mar-r-4"></i>恭喜发表成功，感谢您的支持！</span><div class="comment-content">'+ jQuery.trim(jQuery('#comment').val().replace(/\[S(\d+)\]/g,'<img src="/content/templates/lanye2015/face/$1.gif">').replace(/\[code\]([\s\S]*?)\[\/code\]/g,'<pre0>$1</pre>')) + '</div></div>';
					} else {
						var s = '<div class="comment">';
							s += '<div class="avatar"><img alt="'+ jQuery.trim(jQuery('input[name="comname"]').val()) +'" src="http://gravatar.duoshuo.com/avatar/' + jQuery.md5(jQuery.trim(jQuery('input[name="commail"]').val())) + '?s=48&d=identicon&r=g"></div>';
						s += '<div class="comment-info"><span class="poster"><i class="fa fa-user mar-r-4 green"></i>' + jQuery.trim(jQuery('input[name="comname"]').val()) + '</span><span id="ajax_ok"><i class="fa fa-exclamation-circle mar-r-4"></i>恭喜发表成功，感谢您的支持！</span><div class="comment-content">'+ jQuery.trim(jQuery('#comment').val().replace(/\[S(\d+)\]/g,'<img src="/content/templates/lanye2015/face/$1.gif">').replace(/\[code\]([\s\S]*?)\[\/code\]/g,'<pre>$1</pre>')) + '</div></div>';
					}
					cancelReply();
    ...
    $(document).on('click', '.commt_box #pagenavi a',function(e) {
        ...
        $.ajax({
            ...
            success: function(out) {
                ...
                $("html,body").stop(true);$("html,body").animate({scrollTop: $(".comment-header").offset().top}, 1000);
